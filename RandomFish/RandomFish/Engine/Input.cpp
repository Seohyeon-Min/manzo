...
CS230::Input::Input() {
    ...
    button_down.resize(static_cast<int>(Button::Count));
    previous_button_down.resize(static_cast<int>(Button::Count));
}
...
bool CS230::Input::ButtonDown(Button key)
{
    return button_down[static_cast<int>(key)];
}

bool CS230::Input::ButtonJustPressed(Button key)
{
    return button_down[static_cast<int>(key)] == true && previous_button_down[static_cast<int>(key)] == false;
}

bool CS230::Input::ButtonJustReleased(Button key)
{
    return button_down[static_cast<int>(key)] == false && previous_button_down[static_cast<int>(key)] == true;
}

void CS230::Input::SetButtonDown(Button key, bool value)
{
    button_down[static_cast<int>(key)] = value;
}

Math::vec2 CS230::Input::DeltaMouse()
{
    return { GetMouseDelta().x, GetMouseDelta().y };
}

Math::vec2 CS230::Input::MousePosition()
{
    return { (double)GetMouseX(), (double)GetMouseY() };
}
...
constexpr int cs230_button_to_rl(CS230::Input::Button cs230_key) {
    switch (cs230_key) {
    case CS230::Input::Button::Left:
        return MOUSE_BUTTON_LEFT;

    case CS230::Input::Button::Right:
...
constexpr CS230::Input::Button& operator++(CS230::Input::Button& key) noexcept {
    key = static_cast<CS230::Input::Button>(static_cast<unsigned>(key) + 1);
    return key;
}
...
void CS230::Input::Update() {
    ...
    for (Button button = Button::Left; button < Button::Count; ++button)
    {
        const auto rl_button = cs230_button_to_rl(static_cast<Button>(button));
        SetButtonDown(button, IsMouseButtonDown(rl_button));
    }
}