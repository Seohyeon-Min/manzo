...
RayCasting::RayCasting(float x, float y)
{
    // Set end point relative to mouse cursor
    // Add arbitrary length
    m_relative_end = vec2(x, y) * 3000.f;
}

// Reset end-point of ray
void RayCasting::reset()
{
...
void RayCasting::calc_hit(vec2 p3, vec2 p4)
{
    const vec2 p1 = g_mouse_pos;
    const vec2 p2 = m_end;
