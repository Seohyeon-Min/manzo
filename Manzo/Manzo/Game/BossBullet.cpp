...
void BossBullet::Update(double dt) {
    ...
    if (particle_timer->Remaining() == 0.0) {
        Engine::GetGameStateManager().GetGSComponent<ParticleManager<Particles::bossEbulletParticle>>()->Emit(1, GetPosition(), {0,0}, -GetVelocity() * 0.1f, 1.5);
